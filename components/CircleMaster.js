...
export default function CircleMaster() {
  ...
  // Load leaderboard from blockchain events
  const loadLeaderboard = async () => {
    ...
    try {
      ...
      // If no real data, add test data for UI testing
  ...
  const submitScoreOnchain = async (score) => {
    ...
    try {
      ...
      // Get current gas price and estimate
      const gasPrice = await provider.getGasPrice();
      const gasEstimate = await contract.estimateGas.submitScore(score);
      
      // Reduce gas price by 10% for faster confirmation
      const optimizedGasPrice = gasPrice.mul(90).div(100);
      
      console.log(`Estimated gas: ${gasEstimate.toString()}`);
      console.log(`Gas price: ${ethers.utils.formatUnits(optimizedGasPrice, 'gwei')} gwei`);
      ...
      // Refresh player stats and leaderboard
        ...
        <div className="web3-section mt-4 text-center max-w-lg w-full px-4">
          {!walletConnected ? (
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 space-y-3">
              ...
              {playerStats && playerStats.bestScore > 0 && (
